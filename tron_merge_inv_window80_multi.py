...
"""
高性能版本：解析多个 java-tron stdout 日志目录，按交易哈希聚合输出。

主要特性：
1. 解析阶段按 tx_hash 前缀分桶落盘（bucket）。
2. 排序+导出按 bucket 顺序逐个处理。
...
6. 交易哈希仅匹配同一 peer 在其上方 N 行窗口内最近的一条 INV；超出窗口则视为未匹配。
7. 支持任意多个输入目录；每个输入目录在 workdir 下有独立的 state / parsed / tmp 子目录。

目录结构：
  workdir/
    state/
      <source_id>/progress.json
...
def parse_named_input(spec: str) -> Tuple[Optional[str], str]:
    if "=" in spec:
...
def build_source_id(name: str, folder: Path) -> str:
    slug = sanitize_name(name)
    abs_path = str(folder.resolve())
    digest = hashlib.sha1(abs_path.encode("utf-8", errors="replace")).hexdigest()[:10]
    return f"{slug}__{digest}"



def resolve_input_sources(args: argparse.Namespace) -> List[InputSource]:
    raw_specs: List[str] = []
    if args.input_dir:
        raw_specs.extend(args.input_dir)
    if args.local_dir:
        raw_specs.append(f"local={args.local_dir}")
    if args.remote_dir:
        raw_specs.append(f"remote={args.remote_dir}")

    if not raw_specs:
        raise SystemExit("至少需要提供一个输入目录：使用 --input-dir NAME=PATH（可重复），或旧参数 --local-dir / --remote-dir")

    sources: List[InputSource] = []
    seen_ids: Dict[str, str] = {}
    for idx, spec in enumerate(raw_specs, start=1):
        custom_name, path_str = parse_named_input(spec)
        folder = Path(path_str)
        if not folder.is_dir():
            raise SystemExit(f"输入日志目录不存在: {folder}")

        display_name = custom_name or folder.name or f"source_{idx:02d}"
        source_id = build_source_id(display_name, folder)
        prev = seen_ids.get(source_id)
        if prev is not None and Path(prev).resolve() != folder.resolve():
            raise SystemExit(
                f"输入目录产生了重复的 source_id: {source_id}\n"
                f"请为其中一个目录显式指定不同名字，例如 --input-dir foo={folder}"
            )
        seen_ids[source_id] = str(folder)
        sources.append(InputSource(source_id=source_id, display_name=display_name, folder=folder))
...
def prune_inv_queue(queue: List[Tuple[int, int]], min_line_no: int) -> None:
    while queue and queue[0][0] < min_line_no:
        queue.pop(0)



def prune_recent_inv_by_peer(recent_inv_by_peer: Dict[str, List[Tuple[int, int]]], min_line_no: int) -> None:
    stale_peers = []
    for peer_ip, queue in recent_inv_by_peer.items():
        prune_inv_queue(queue, min_line_no)
        if not queue:
            stale_peers.append(peer_ip)
    for peer_ip in stale_peers:
        recent_inv_by_peer.pop(peer_ip, None)
...
def parse_one_source(
    ...
    progress_lines: int,
    resume: bool,
    inv_window_lines: int,
) -> dict:
    t0 = time.time()
    folder_p = Path(folder)
    workdir_p = Path(workdir)
    log_name = f"{display_name}|{source}"

    files = discover_log_files(folder_p, source)
    if not files:
        eprint(f"[WARN] [{log_name}] 未发现可解析日志: {folder_p}")
        return {"source": source, "display_name": display_name, "files": 0, "matched": 0, "unmatched": 0, "inv": 0}

    eprint(f"[INFO] [{log_name}] 开始处理目录: {folder_p}")
    eprint(f"[INFO] [{log_name}] 共发现 {len(files)} 个日志文件")
    ...
    recent_inv_by_peer: Dict[str, List[Tuple[int, int]]] = {}
    global_line_no = 0
    start_idx = 0
    ...
    if resume and progress_path.exists():
        ...
        if progress is not None:
            ...
            if version < 3:
                raise RuntimeError(
                    f"[{log_name}] 检测到旧版断点状态，不包含当前多目录 + INV 窗口匹配所需信息，无法安全续跑。"
                    f"请使用 --reset 重新开始。"
                )
            completed_keys = list(progress.get("completed_keys", []))
            start_idx = validate_resume_prefix(files, completed_keys, log_name)
            global_line_no = int(progress.get("global_line_no", 0))
            recent_raw = progress.get("recent_inv_by_peer", {})
            for peer_ip, items in recent_raw.items():
                queue: List[Tuple[int, int]] = []
                for item in items:
                    if isinstance(item, (list, tuple)) and len(item) >= 2:
                        queue.append((int(item[0]), int(item[1])))
                if queue:
                    recent_inv_by_peer[peer_ip] = queue
            prune_recent_inv_by_peer(recent_inv_by_peer, global_line_no - inv_window_lines)
            eprint(f"[INFO] [{log_name}] 从断点续跑，已完成文件数={start_idx}")
    else:
        eprint(f"[INFO] [{log_name}] 从头开始处理")
    ...
    for idx in range(start_idx, len(files)):
        ...
        eprint(f"[INFO] [{log_name}] 开始处理文件 {idx + 1}/{len(files)}: {meta.path.name}")
        if meta.is_current:
            eprint(f"[INFO] [{log_name}] 当前文件使用 mtime 推断日期: {meta.path.name} -> {meta.date_str}")
        ...
        try:
                ...
                for raw in f:
                    ...
                    if rest.startswith("INV "):
                        ...
                        if len(parts) >= 3 and parts[0] == "INV" and is_ipv4(parts[1]) and parts[2].isdigit():
                            peer_ip = parts[1]
                            queue = recent_inv_by_peer.setdefault(peer_ip, [])
                            queue.append((global_line_no, int(parts[2])))
                            prune_inv_queue(queue, global_line_no - inv_window_lines)
                            file_stats["inv"] += 1
                            total_stats["inv"] += 1
                        else:
                    ...
                    if first_token and first_token[0].isdigit():
                        ...
                        if len(parts) >= 2 and is_ipv4(parts[0]) and is_hex_hash(parts[1]):
                            ...
                            queue = recent_inv_by_peer.get(peer_ip)
                            inv_size = None
                    ...
                    if progress_lines > 0 and file_stats["lines"] % progress_lines == 0:
                        eprint(
                            f"[INFO] [{log_name}] 文件进行中: {meta.path.name} | "
            ...
            progress = {
                ...
                "inv_window_lines": inv_window_lines,
                "recent_inv_by_peer": {
                    peer_ip: [[line_no, inv_size] for line_no, inv_size in queue]
                    for peer_ip, queue in recent_inv_by_peer.items()
                    if queue
                },
            ...
            eprint(
                f"[INFO] [{log_name}] 文件完成 {idx + 1}/{len(files)}: {meta.path.name} | "
    ...
    eprint(
        f"[INFO] [{log_name}] 全部完成 | 文件数={total_stats['files']} | 已处理到={total_stats['processed_files']} | "
    ...
    return {"source": source, "display_name": display_name, **total_stats, "elapsed": elapsed_all}
...
def iter_parsed_source_dirs(workdir: Path) -> Iterable[Path]:
    parsed_root = workdir / "parsed"
    if not parsed_root.exists():
        return []
    return sorted([p for p in parsed_root.iterdir() if p.is_dir()], key=lambda p: p.name)



def list_bucket_shards(workdir: Path, bucket: str) -> List[Path]:
    files: List[Path] = []
    for source_dir in iter_parsed_source_dirs(workdir):
        files.extend(sorted(source_dir.glob(f"*.{bucket}.tsv")))
    return files
...
def cleanup_bucket_artifacts(workdir: Path, bucket: str) -> int:
    ...
    freed = 0
    for source_dir in iter_parsed_source_dirs(workdir):
        for p in source_dir.glob(f"*.{bucket}.tsv"):
            try:
                size = p.stat().st_size
    ...
    sorted_file = workdir / "sorted" / f"bucket_{bucket}.sorted.tsv"
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="高性能合并多个 java-tron stdout 日志目录，按交易哈希输出块文本（内建按 bucket 自动 GC）",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="输入日志目录。可重复使用。支持 PATH 或 NAME=PATH 两种形式；建议多目录时使用 NAME=PATH 以便识别。",
    )
    parser.add_argument("--local-dir", help="兼容旧参数：本地日志目录")
    parser.add_argument("--remote-dir", help="兼容旧参数：远程同步下来的日志目录")
    parser.add_argument("--parse-workers", type=int, default=max(1, min(cpu, 4)), help="解析阶段并行处理输入目录的进程数")
    ...
    parser.add_argument("--inv-window-lines", type=int, default=80, help="交易哈希向上匹配同一 peer 的 INV 时允许的最大行距")
...
def main() -> int:
    ...
    sources = resolve_input_sources(args)
    ...
    eprint(f"[INFO] [main] 输入目录数={len(sources)} | parse_workers={min(max(1, args.parse_workers), len(sources))}")
    for src in sources:
        eprint(f"[INFO] [main] source={src.display_name} | id={src.source_id} | folder={src.folder}")

    overall_start = time.time()

    parse_start = time.time()
    max_workers = min(max(1, args.parse_workers), len(sources))
